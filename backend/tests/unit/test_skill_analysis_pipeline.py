...
async def test_market_variant_computes_hard_gaps_without_llm_or_graphrag(
    ...
    with patch.object(kg_repository, "get_person_skills", new_callable=AsyncMock) as mock_skills:
        ...
        with patch.object(kg_repository, "query", new_callable=AsyncMock) as mock_query:
            ...
            with patch.object(graph_rag, "retrieve", new_callable=AsyncMock) as mock_retrieve:
...
async def test_detects_underleveled_skill(
    ...
    with patch.object(kg_repository, "get_person_skills", new_callable=AsyncMock) as mock_skills:
        ...
        with patch.object(kg_repository, "query", new_callable=AsyncMock) as mock_query:
            mock_query.return_value = [
                {"name": "Python", "demand": 10, "levels": ["advanced"] * 8 + ["intermediate"] * 2},
            ]
...
async def test_target_role_not_ready_surfaces_status_without_fake_gaps(
    ...
    with patch.object(kg_repository, "get_person_skills", new_callable=AsyncMock) as mock_skills:
        ...
        with patch.object(kg_repository, "query", new_callable=AsyncMock) as mock_query:
            mock_query.return_value = [
                {"title": "Rare Role", "sample_status": "scraping", "sample_job_count": 0}
            ]

            result = await skill_analysis_pipeline.run(user_id, target_role_id=target_role_id)
...
async def test_target_role_missing_reports_not_ready(
    ...
    with patch.object(kg_repository, "get_person_skills", new_callable=AsyncMock) as mock_skills:
        ...
        with patch.object(kg_repository, "query", new_callable=AsyncMock) as mock_query:
            ...
            result = await skill_analysis_pipeline.run(user_id, target_role_id="missing_role")
                ...
                with patch.object(
                    ...
                    with patch(
                        ...
                        roadmap = await learning_roadmap_pipeline.generate(
                            user_id, skills=["Kubernetes"], target_role_id=target_role_id
                        )
