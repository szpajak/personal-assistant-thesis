...
function subtitleOf(data: KgEntityData): string | null {
  ...
  if (data.kgType === 'Project' && data.status) return String(data.status).replaceAll('_', ' ');
  ...
  if (data.kgType === 'Certificate') return data.issuer ? String(data.issuer) : null;
  ...
  if (data.kgType === 'Education') return data.degree ? String(data.degree) : null;
  return null;
}

export const KgEntityNode = memo(({ data, selected }: NodeProps<KgEntityData>) => {
  ...
  const muted = data.kgType === 'Project' && data.status && data.status !== 'finished';
  const outline = data.kgType === 'Skill' && data.hasDirectSkillLink === false;
  ...
  return (
    <div
      ...
      style={{
        ...
        minWidth: 220 * scale,
        maxWidth: 260,
        boxShadow: selected || data.focused ? `0 0 0 2px ${theme.hex}` : undefined,
      }}
    >
      <Handle type="target" position={Position.Left} className="!h-2 !w-2" style={{ background: theme.hex }} />
      <Handle type="source" position={Position.Right} className="!h-2 !w-2" style={{ background: theme.hex }} />
      <div className="flex items-center gap-2 px-2.5 py-1.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: theme.hex }}>
              {typeLabel}
            </span>
          ...
          <div className="truncate text-sm font-semibold leading-tight">{titleOf(data)}</div>
          {subtitleOf(data) && (
            <div className="truncate text-[10px] capitalize text-muted-foreground">{subtitleOf(data)}</div>
          )}
        </div>
      </div>
    </div>
...
export const KgRegionNode = memo(({ data }: NodeProps<{ label: string; regionId: string }>) => {
