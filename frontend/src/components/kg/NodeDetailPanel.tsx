...
import { curatedFields, displayName, FAMILY_THEME, typeHref, TYPE_LABELS } from './graphModel';
...
export function NodeDetailPanel({ node, enriched, onClose, onFocus }: NodeDetailPanelProps) {
  ...
  const fields = enriched
    ...
    : Object.entries(node.data)
        .filter(([key]) => !['label', 'kgType', 'family', 'embedding', 'focused', 'dimmed', 'highlighted'].includes(key))
        .filter(([, value]) => value !== undefined && value !== null && value !== '')
        .map(([key, value]) => ({ key, label: key, value }));

  const url = typeof node.data.url === 'string' ? node.data.url : undefined;
  const website = typeof node.data.website === 'string' ? node.data.website : undefined;
  ...
  return (
    <Card className="absolute right-4 top-4 z-20 w-80 max-h-[calc(100%-2rem)] shadow-lg">
      <CardHeader className="flex flex-row items-start justify-between space-y-0 border-b py-3">
        <div>
          <CardTitle className="text-xs font-bold uppercase tracking-widest" style={{ color: theme.hex }}>
            {TYPE_LABELS[kgType] ?? kgType}
          </CardTitle>
          <div className="mt-1 text-sm font-semibold leading-snug">
            {enriched ? displayName(enriched) : String(node.data.name || node.data.title || node.data.label || '')}
          </div>
        </div>
                ...
                {Array.isArray(field.value) ? (
                  <div className="flex flex-wrap gap-1">
                    {field.value.map((item, index) => (
                      <Badge key={`${field.key}-${index}`} variant="secondary" className="text-[10px] font-normal">
                        {String(item)}
                      </Badge>
                    ))}
                  </div>
                ) : (
                  <div className="break-words text-sm font-medium">{String(field.value)}</div>
                )}
              </div>
            ))}
            ...
            <div className="flex flex-col gap-2 pt-1">
              ...
              {href && (
                <Button size="sm" variant="outline" asChild>
                  <Link href={href}>Open {TYPE_LABELS[kgType] ?? kgType} page</Link>
                </Button>
              )}
              {link && (
                <Button size="sm" variant="outline" asChild>
