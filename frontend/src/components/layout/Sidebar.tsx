  ...
  return (
    <div className="flex h-full w-60 flex-col bg-gray-950 text-white">
      <div className="flex h-16 items-center px-6">
        <Link href="/" className="flex items-center space-x-2" onClick={onClose} aria-label="Career AI Home">
      ...
      <div className="border-t border-white/10 p-4 space-y-3">
        <div className="flex items-center space-x-3 px-2">
          <div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center font-bold text-xs" aria-hidden="true">
            {user?.full_name ? getInitials(user.full_name) : "U"}
          </div>
          <div className="flex-1 overflow-hidden text-xs">
            <p className="truncate font-medium">{user?.full_name || "Loading..."}</p>
