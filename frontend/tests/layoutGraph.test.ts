...
import {
  getRadialLayout,
  getSubstrateLayout,
} from "../src/components/kg/layoutGraph";
...
const payload: KGGraphPayload = {
  nodes: [
    ...
    },
    {
      ...
      data: {
        label: "Python",
        properties: { name: "Python", category: "backend" },
      },
    },
    {
      id: "proj_1",
...
describe("layoutGraph", () => {
  ...
  it("puts past employers in Evidence and hiring companies in Organizations", () => {
    ...
    expect(substrateRegionFor(nodes.find((node) => node.id === "acme")!)).toBe(
      "evidence",
    );
    expect(substrateRegionFor(nodes.find((node) => node.id === "mega")!)).toBe(
      "orgs",
    );
    expect(substrateRegionFor(nodes.find((node) => node.id === "both")!)).toBe(
      "evidence",
    );
    ...
    expect(
      laid.nodes
        .filter((node) => node.type === "kgRegion")
        .map((node) => node.id),
    ).toEqual([
      "region-evidence",
      "region-skills",
    ...
    expect(laid.nodes.find((node) => node.id === "acme")?.parentNode).toBe(
      "region-evidence",
    );
    expect(laid.nodes.find((node) => node.id === "both")?.parentNode).toBe(
      "region-evidence",
    );
    expect(laid.nodes.find((node) => node.id === "mega")?.parentNode).toBe(
      "region-orgs",
    );
    expect(laid.nodes.find((node) => node.id === "job_1")?.parentNode).toBe(
      "region-market",
    );
    expect(laid.nodes.some((node) => node.data?.label === "Employers")).toBe(
      true,
    );
  });
