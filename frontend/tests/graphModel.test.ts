...
const payload: KGGraphPayload = {
  ...
  nodes: [
    { id: 'user_1', type: 'Person', data: { label: 'Ada', properties: { name: 'Ada' } } },
    {
      id: 'skill_1',
      type: 'Skill',
      data: { label: 'Python', properties: { name: 'Python', level: 'expert', category: 'backend' } },
    },
    { id: 'proj_1', type: 'Project', data: { label: 'App', properties: { title: 'App' } } },
    { id: 'job_1', type: 'JobOffer', data: { label: 'SWE', properties: { title: 'SWE' } } },
    { id: 'app_1', type: 'Application', data: { label: 'Applied', properties: { status: 'Applied' } } },
  ],
  edges: [
...
describe('graphModel', () => {
  ...
  it('hides Person, applications, and HAS_SKILL on the default career map', () => {
    ...
    expect(visible.visibleEdges.map((edge) => edge.label)).toEqual(['USES', 'REQUIRES']);
  });
  ...
  it('keeps a 1-hop neighborhood in focus view', () => {
    ...
    expect(visible.visibleNodes.map((node) => node.id).sort()).toEqual(['job_1', 'proj_1', 'skill_1']);
  });
  ...
  it('collapses crowded skill lists until expanded', () => {
    const { nodes } = enrichGraph({
      nodes: Array.from({ length: SKILL_COLLAPSE_THRESHOLD + 3 }, (_, index) => ({
        id: `s${index}`,
        type: 'Skill',
        data: { label: `S${index}`, properties: { name: `S${index}`, level: 'beginner' } },
      })),
      edges: [],
    });
  ...
  it('keeps owned skills and their neighborhood, including posting companies', () => {
    const { nodes, edges } = enrichGraph({
      ...
      nodes: [
        { id: 'user_1', type: 'Person', data: { label: 'Ada', properties: { name: 'Ada' } } },
        { id: 'python', type: 'Skill', data: { label: 'Python', properties: { name: 'Python' } } },
        { id: 'k8s', type: 'Skill', data: { label: 'K8s', properties: { name: 'K8s' } } },
        { id: 'proj_1', type: 'Project', data: { label: 'App', properties: { title: 'App' } } },
        { id: 'job_1', type: 'JobOffer', data: { label: 'SWE', properties: { title: 'SWE' } } },
        { id: 'mega', type: 'Company', data: { label: 'Mega', properties: { name: 'Mega' } } },
      ],
    ...
    const owned = applyVisibility(nodes, edges, { ...defaultFilters(), skillOwnership: 'owned' });
    ...
    const gap = applyVisibility(nodes, edges, { ...defaultFilters(), skillOwnership: 'gap' });
